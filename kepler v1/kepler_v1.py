...
#Solar System class holds planets and star

class SolarSystem():
    def __init__(self, name, planet_list, star):
        self.name = name
    ...
    #Displays the information about the solar system
    def display_info(self):
        sun = self.star
        print("---------------------------------------------------------------")
        print("Displaying Solar System Info for", self.name)
        print("---------------------------------------------------------------")
        print("")
...
class Star():
    def __init__(self, name, mass, temperature, radius, sun_list):
        #Define properties of the star
        self.name = name
        self.mass = mass
        self.radius = radius
        self.temperature = temperature
        self.luminosity = constants["sigma"] * 4 * pi * (self.radius ** 2) * (self.temperature ** 4)
    ...
    def display_info(self):
        #Displays information of the star
        print("---------------------------------------------------------------")
        print("Displaying Star Info for", self.name)
        print("---------------------------------------------------------------")
        print("")
        print("Name:  ", self.name)
...
class Planet:
    def __init__(self, name, mass, radius, distanceFromSun, planets, star):
        ...
        self.colour = [0,0,0]
        self.intensity = (star.luminosity) / (4 * pi * (self.distanceFromSun ** 2))
        self.dwarf = False
    ...
    def graph(self, quantity):
        ...
        if quantity == "displacement":
            ...
            plt.plot(t, (r * np.sin(w * t)), label = "y-component")
            ...
            plt.plot(t, (r * w * np.cos(w * t)), label = "y-component")
            ...
            plt.plot(t, (r * (w**2) * -1 * np.sin(w * t)), label = "y-component")
    ...
    def display_info(self):
        ...
        print("Dwarf: ", self.isDwarf())
        print("")
        print("---------------------------------------------------------------")

#Use INHERITANCE to create a new object if the mass of the planet is really small
...
def add(planets, sun):
    if len(planets) < constants["planet capacity"]: #Checks that the planet LIST isn't already full yet
        ...
        if mass <= constants["dwarf mass"]:
            vars()[temp] = Dwarf(name, mass, radius, distance_from_sun, planets, sun)
        else:
            vars()[temp] = Planet(name, mass, radius, distance_from_sun, planets, sun)
        print("Planet Added")
    else:
...
def change_sun(sun, min_mass, sun_list):
    #First displays info for the star
    sun.display_info()
    #Deletes star to reinstate with new information
    del sun
    mass = 0
    valid = False
    #Takes new information if valid
    while valid == False:
        try:
            ...
            radius = float(input("Enter the radius of the star: "))
        except ValueError:
            print("Please enter valid data.")
            valid = False
    #Creates new star
    sun = Star(name, mass, temperature, radius, sun_list)
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                ...
                if (475 < mouse_x < 625) and (470 < mouse_y < 500) and (welcome == True):
                    ...
                    sun = Star("Sun", 2e30, 5.778e3, 695.51e6, sun_list)
                    earth = Planet("Earth", 6e24, 6e6, 150e9, planets, sun)
                    uranus = Planet("Uranus", 9e25, 25362000, 3e12, planets, sun)
                    mySystem = SolarSystem("My Solar System", planets, sun)
                    mySystem.display_info()
                    welcome = False
                    ...
                    if default == True:
                        if (885 < mouse_x < 915) and (50 < mouse_y < 80): #Checks if the play button has been clicked
                            ...
                            if len(planets) == 0:
                                #First checks if there are any planets
